...
class DefaultController extends BackendController
{
	...
	public function accessRules()
	{
		return array_merge(
				[
				['allow','actions'=>['index','create','update','delete','move'],'users'=>['*']],
				['deny'],
				],
				parent::accessRules()
		);
	...
	public function actionIndex()
	{
		...
		if(isset($_POST['Lookup']))
		{
			...
			if($model->save()){
				Yii::app()->user->setFlash('success','数据保存成功！');
				$this->redirect(array('index','type'=>$model->type));
			}
		}
		
		$adminmodel=new Lookup('search');

		if(!empty($_GET['type']))
			$adminmodel->type=$_GET['type'];
		else
			$adminmodel=null;
		$this->render('index',array(
			'model'=>$model,
			'adminMode'=>$adminmodel
		));
	}
	/**
	 * Creates a new model.
